...
const SvgToCssPage: React.FunctionComponent = () => {
	...
	const handleInputChange = (event: any) => {
		...
		if ((event.target.name = "svgCode")) {
			if (checkIfValidSVG(event.target.value)) {
				...
				setPureDataOutput(
					`"data:image/svg+xml,${inverseQuoteType(
						encodeSVG(event.target.value),
						"single"
					)}"`
				)
				setCssBackgroundOutput(
					`background-image: url("data:image/svg+xml,${inverseQuoteType(
						encodeSVG(event.target.value),
						"single"
					)}");`
				)
	...
	return (
		<Layout
			current="SVG to CSS"
			title="SVG to CSS | DVLP HAUS | toolbox for developers"
		>
			<Container>
					...
					<div>
						<div className="input_field">
							<div className="input_field_container">
								...
								<div
									className="svg__preview"
									dangerouslySetInnerHTML={{
										__html: `${
											svgPreview
												? svgPreview.toString().includes("<svg") &&
												  svgPreview.toString().includes("</svg>")
													? svgPreview
													: ""
												: ""
